...
export default function RelatoriosGestorPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!gestorSetores?.length) { setLoading(false); return }
  ...
  const candidatosFiltrados = setorSelecionado
    ...
    : candidatos
  const topCandidatos = candidatosFiltrados.sort((a, b) => (b.match_score || 0) - (a.match_score || 0)).filter(c => c.match_score !== null)
  const TROPHY_COLORS = ['#F59E0B', '#94A3B8', '#B45309']
  const BORDER_CLASSES = ['border-[#F59E0B]', 'border-[#94A3B8]', 'border-[#B45309]']

  if (loading) return <div className="flex items-center justify-center h-96"><div className="w-8 h-8 border-2 border-[#00D4FF]/20 border-t-[#00D4FF] rounded-full animate-spin" /></div>
      ...
      {setores.length > 0 && (
        <div className="bg-[#111633] border border-[#1e2a5e] rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-semibold text-white">Meus Setores</span>
          </div>
            ...
            {topCandidatos.slice(0, 3).map((c, i) => (
              <div key={c.id} className={`bg-[#111633] border-2 ${BORDER_CLASSES[i]} rounded-xl p-4 text-center`}>
                ...
                <div className="space-y-2">
                  <div className="flex justify-center"><MatchScoreBadge score={c.match_score} /></div>
